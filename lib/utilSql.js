...
class Connex2db {
  ...
  sqlAddDept({longName, shortName}) {
   ...
   sqlAddEmployee({firstName, lastName, roleId, mgrId}) {
    ...
    return this.connex2db.promise().query(cmdInsert,[firstName, lastName, roleId, mgrId]).then(data => {
      // console.log(data);
      return data;
    }).catch(err => {
      console.log(err)
  ...
  sqlEmpChoices() {
    ...
    const cmdSelect =  `SELECT 
    emp.id as value,
    CONCAT (emp.first_name, " ", emp.last_name, " in ", dept.name_long) AS name
  FROM employee emp
  JOIN role
    ON role.id = emp.role_id
  JOIN department dept
  ...
  ORDER BY mgr.manager_id;`; 
    ...
    *
    ...
    return this.connex2db.promise().query(cmdSelId,params).then(data => {
      // console.log(data);
      return data;
    }).catch(err => {
      console.log(err)
   ...
   sqlUpdOneDept({ deptId, longName, shortName }) {
    //  const cmdUpdate = `INSERT INTO department(dept_id,long_name, short_name) 
    // VALUES (?,?,?)
    // ON DUPLICATE KEY UPDATE department SET 
    ...
    return this.connex2db.promise().query(cmdUpdate,params).then(data => {
      // console.log(data);
      return data;
    }).catch(err => {
      console.log(err)
